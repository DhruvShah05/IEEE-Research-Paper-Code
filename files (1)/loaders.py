...
# (Ntakaris et al. 2018, Zhang et al. 2019):
#
#   canonical col 4i+0 : ask_price_(i+1)
#   canonical col 4i+1 : ask_vol_(i+1)
#   canonical col 4i+2 : bid_price_(i+1)
#   canonical col 4i+3 : bid_vol_(i+1)          for i = 0..9   (40 cols)
#
# Crypto raw layout (parquet cols '2'..'41', see scripts/prepare_crypto.py and
# data/features.py::to_relative_price — prices/volumes ALTERNATE within each side):
#
#   X col 2i   : bid_price_(i+1)     (parquet col 2+2i)
#   X col 2i+1 : bid_vol_(i+1)       (parquet col 3+2i)     for i = 0..9   -> cols 0..19
#   X col 20+2i: ask_price_(i+1)     (parquet col 22+2i)
#   X col 21+2i: ask_vol_(i+1)       (parquet col 23+2i)    for i = 0..9   -> cols 20..39
#
# FI-2010 (both full144 and raw40) is already in the canonical layout.

CRYPTO_TO_CANONICAL_PERM = []
for _i in range(10):
    CRYPTO_TO_CANONICAL_PERM += [20 + 2 * _i, 21 + 2 * _i, 2 * _i, 2 * _i + 1]
CRYPTO_TO_CANONICAL_PERM = np.array(CRYPTO_TO_CANONICAL_PERM, dtype=np.int64)
CANONICAL_TO_CRYPTO_PERM = np.argsort(CRYPTO_TO_CANONICAL_PERM)


def reorder_to_canonical(X: np.ndarray, market: str) -> np.ndarray:
    """
    Reorders the first 40 raw LOB columns of X into the canonical interleaved
    layout used by DeepLOB and level/grouped-tokenised Transformers:

        [ask_p1, ask_v1, bid_p1, bid_v1, ask_p2, ask_v2, bid_p2, bid_v2, ...]

    FI-2010 is already in this layout and is returned unchanged.
    Crypto is reordered from its side-blocked, price/volume-alternating layout.
    Columns beyond 40 (e.g. FI-2010 derived features) pass through unchanged.

    Works on 2-D (N, F) arrays and on 3-D (N, T, F) windowed arrays.
    """
    if market != 'crypto':
        return X
    if X.shape[-1] < 40:
        raise ValueError(f"reorder_to_canonical expects >= 40 feature columns, got {X.shape[-1]}")
    X_new = X.copy()
    X_new[..., :40] = X[..., CRYPTO_TO_CANONICAL_PERM]
    return X_new


def reorder_from_canonical(X: np.ndarray, market: str) -> np.ndarray:
    """Inverse of reorder_to_canonical (used in tests)."""
    if market != 'crypto':
        return X
    X_new = X.copy()
    X_new[..., :40] = X[..., CANONICAL_TO_CRYPTO_PERM]
    return X_new
